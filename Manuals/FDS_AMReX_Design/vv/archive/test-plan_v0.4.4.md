# FDS-AMR Verification & Validation Test Plan (DRAFT v0.4.4)
...
**Case list.** The FR-006 case list is every row of `docs/vv/scope_case_list.csv` with `cls = IN`. That is 693 of the 941 `Verification/**/*.fds` inputs at FireX `36975d765f`. The other classes:

| cls | Count | Meaning |
|---|---|---|
| IN | 693 | All features in scope and runnable here |
| DEFERRED | 175 | Uses GEOM, CC_IBM (MISC or implied by GEOM) or HT3D |
| OUT | 39 | TRN stretching (IR-002/D-030); GPU (`GPU_Tests/`, explicit `HYPRE_DEVICE_RUN=T`); the four decided embedded/overlapping cases; VTK (D-019); missing external files |
| UNCLEAR | 34 | Needs a decision; reason in the `reason` column |
...
| CYLINDRICAL | 11 | Q4 |
| TUNNEL_PRECONDITIONER | 6 | Q4 |
| ZONE leakage over several meshes | 2 | FR-034 addendum |
| Thin OBST at a static coarse/fine interface | 1 | FR-040 R3 ruling |
