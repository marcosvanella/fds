         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               ELSE
                  ...
                  DZZ=ZZ_GET
                  CALL COMBUSTION_MODEL(T,DT,ZZ_GET,CC%Q(JCC),CV%MIX_TIME(CC%IG(JCC)),CV%CHI_R(CC%IG(JCC)),&
      ...
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO JCC=1,CC%NCELL
            ...
            ELSEIF (.NOT.(CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY)) THEN
               ...
               CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(CC%IG(JCC))*CC%Q(JCC)*CC%VOLUME(JCC)
            ENDIF
         ENDDO
...
DZZ_IF2: IF ( ANY(ABS(DZZ) > DZZ_CLIP) ) THEN
   ...
   ELSE
      ...
      DO N=1,N_TRACKED_SPECIES
         ...
         CUT_CELL(ICC)%M_DOT_PPP(N,JCC)=CUT_CELL(ICC)%M_DOT_PPP(N,JCC)+CUT_CELL(ICC)%RHO(JCC)*DZZ(N)/DT
      ENDDO
   ENDIF
ENDIF DZZ_IF2
