...
INTEGER :: N_REQ0, STARTLOC, ENDLOC
...
IF (SORTED_RANK_INDX_MAP(MY_RANK+1) < AVG_PIVOT_INDX) THEN
   ...
   DO NP =1, TOT_RANK_TO_COMM
      ...
      ENDIF
      ENDLOC = NCELLS_TO_SEND_ARRAY_ALGO2(NP,3)
      CALL MPI_ISEND(REALS_TO_SEND_ARRAY(:,STARTLOC:ENDLOC),NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)*NVAR_TO_SEND, &
...
ELSE IF(SORTED_RANK_INDX_MAP(MY_RANK+1) >= AVG_PIVOT_INDX) THEN ! Only recieve.
   ...
   ! Now recieve
   ALLOCATE(REALS_TO_RECV_ARRAY(NVAR_TO_SEND,TOT_CELLS_TO_RECEIVE)); REALS_TO_RECV_ARRAY=0._EB
   ALLOCATE(REQ0(TOT_RANK_TO_COMM))
   N_REQ0 = 0
   DO NP =1, TOT_RANK_TO_COMM
      N_REQ0 = N_REQ0 + 1;
      ...
      ENDIF
      ENDLOC = NCELLS_TO_RECV_ARRAY_ALGO2(NP,3)
      CALL MPI_IRECV(REALS_TO_RECV_ARRAY(:,STARTLOC:ENDLOC),NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)*NVAR_TO_SEND, &
               MPI_DOUBLE_PRECISION,NCELLS_TO_RECV_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO

   NRECEIVE_CELLS=TOT_CELLS_TO_RECEIVE
...
IF (SORTED_RANK_INDX_MAP(MY_RANK+1) < AVG_PIVOT_INDX) THEN ! Recieve the chemistry cells 
   ...
   DO NP =1, TOT_RANK_TO_COMM
      ...
      ENDIF
      ENDLOC = NCELLS_TO_SEND_ARRAY_ALGO2(NP,3)
      CALL MPI_IRECV(RESULTS_TO_RECV_ARRAY(:,STARTLOC:ENDLOC),NCELLS_TO_SEND_ARRAY_ALGO2(NP,2)*NVAR_TO_RECEIVE, &
            MPI_DOUBLE_PRECISION,NCELLS_TO_SEND_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO
...
ELSE IF(SORTED_RANK_INDX_MAP(MY_RANK+1) >= AVG_PIVOT_INDX) THEN ! Send the chemistry cells
   ...
   DO NP =1, TOT_RANK_TO_COMM
      ...
      ENDIF
      ENDLOC = NCELLS_TO_RECV_ARRAY_ALGO2(NP,3)
      CALL MPI_ISEND(RESULTS_TO_SEND_ARRAY(:,STARTLOC:ENDLOC),NCELLS_TO_RECV_ARRAY_ALGO2(NP,2)*NVAR_TO_RECEIVE, &
            MPI_DOUBLE_PRECISION,NCELLS_TO_RECV_ARRAY_ALGO2(NP,1),TAG,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
   ENDDO
ENDIF
IF (N_REQ0>0) CALL MPI_WAITALL(N_REQ0,REQ0(1:N_REQ0),MPI_STATUSES_IGNORE,IERR)
...
IF (SORTED_RANK_INDX_MAP(MY_RANK+1) < AVG_PIVOT_INDX) THEN ! Populate correct varaibles
   ! Set appropriate variables
   !---------------------------------------------
   REAC_SOURCE_TERM_TMP = 0._EB
   Q_REAC_TMP           = 0._EB
