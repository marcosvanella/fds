...
INTEGER :: NM,I,J,K,CHEM_SUBIT_TMP,CHEM_SUBIT_TMP_OUT,ICC,JCC,INDEX,NRECEIVE_CELLS
...
!------,MPI_COMM_WORLD,IERR
 ...
 DO_CHEM_LOAD_BALANCE_IF : IF(N_MPI_PROCESSES > 1 .AND. DO_CHEM_LOAD_BALANCE) THEN
   ...
   CALL MPI_ALLREDUCE(NCHEM_ACTIVE_CELLS, NCHEM_ACTIVE_CELLS_GLOBAL, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, IERR)
   
   IF (NCHEM_ACTIVE_CELLS_GLOBAL > 0) THEN

      ! Distribute chemically activated cell
      CALL DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES(NCHEM_ACTIVE_CELLS, NRECEIVE_CELLS)
      
      ! Call chemistry
      ALLOCATE(RESULTS_TO_SEND_ARRAY(NVAR_TO_RECEIVE, NRECEIVE_CELLS)); RESULTS_TO_SEND_ARRAY=0._EB
      ...
      CALL GATHER_CELLS_FROM_MPI_PROCESSES(NCHEM_ACTIVE_CELLS, DT)
      IF (STOP_STATUS/=NO_STOP) RETURN
...
SUBROUTINE DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES (NCHEM_ACTIVE_CELLS, NRECEIVE_CELLS)
INTEGER, INTENT(IN) :: NCHEM_ACTIVE_CELLS
INTEGER, INTENT(OUT) :: NRECEIVE_CELLS
INTEGER :: I, J , K, NM, INDEX, ICC, JCC, RANK_TO, SEQ,IERR
...
CELLS_ALLOC    = MAX(CELLS_PER_RANK * N_MPI_PROCESSES,1)

ALLOCATE(NCELLS_TO_SEND_ARRAY(N_MPI_PROCESSES)); NCELLS_TO_SEND_ARRAY=0
ALLOCATE(REALS_TO_SEND_ARRAY(NVAR_TO_SEND,CELLS_ALLOC)); REALS_TO_SEND_ARRAY=0._EB
ALLOCATE(CHEM_SEQ_ARR(CELLS_ALLOC));
   
SEQ = 0 ! Store the sequence
! Prepare the Send buffer data
IF (NCHEM_ACTIVE_CELLS > 0) THEN
         ...
         DO J=1,JBAR
            DO I=1,IBAR
               IF(CHEM_ACTIVE(I,J,K)) THEN
                  ...
                  INDEX= CELLS_PER_RANK*(RANK_TO-1) + NCELLS_TO_SEND_ARRAY(RANK_TO)
                  INTEGERS_TEST_ARRAY(INDEX) = NM
                  CHEM_SEQ_ARR(SEQ) = INDEX
                  ! Reals to send:
                  ZZ_GET = ZZ(I,J,K,1:N_TRACKED_SPECIES)
                  PRES = PBAR(K,PRESSURE_ZONE(I,J,K)) + RHO(I,J,K)*(H(I,J,K)-KRES(I,J,K))
   ...
   SEQ=0
   DO INDEX=1,CELLS_ALLOC
      IF(INTEGERS_TEST_ARRAY(INDEX) == -1) CYCLE
      SEQ=SEQ+1
      NEW_INDEX(SEQ)=INDEX
      REV_NEW_INDEX(INDEX) = SEQ
   ENDDO
...
ENDDO
NRECEIVE_CELLS=MAX(SUM(NCELLS_TO_RECV_ARRAY),1)
ALLOCATE(REALS_TO_RECV_ARRAY(NVAR_TO_SEND,NRECEIVE_CELLS)); REALS_TO_RECV_ARRAY=0._EB
...
SUBROUTINE GATHER_CELLS_FROM_MPI_PROCESSES(NCHEM_ACTIVE_CELLS, DT)
INTEGER, INTENT(IN) :: NCHEM_ACTIVE_CELLS
REAL(EB), INTENT(IN) :: DT
INTEGER :: I, J , K, NM, INDEX, ICC, JCC, IERR, SEQ
INTEGER :: RECIEVE_SIZE, CHEM_SUBIT_TMP
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES), &
...
RECIEVE_SIZE = MAX(NCHEM_ACTIVE_CELLS,1)
SEND_DISPS(1) = 0; REVC_DISPS(1) = 0
DO I=2,N_MPI_PROCESSES
...
ENDDO
ALLOCATE(RESULTS_TO_RECV_ARRAY(NVAR_TO_RECEIVE,RECIEVE_SIZE)); RESULTS_TO_RECV_ARRAY=0._EB
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF(CHEM_ACTIVE(I,J,K)) THEN
               SEQ=SEQ+1
               INDEX = CHEM_SEQ_ARR(SEQ)
               DZZ                  = ZZ(I,J,K,1:N_TRACKED_SPECIES)
               ZZ_GET               = RESULTS_TO_RECV_ARRAY(1:N_TRACKED_SPECIES,INDEX)
