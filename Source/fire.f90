         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               IF (ICV>0) THEN
                  ...
                  ZZ_GET=CV%ZZ(1:N_TRACKED_SPECIES,ICV)
                  DZZ=ZZ_GET
                  CALL COMBUSTION_MODEL(T,DT,ZZ_GET,CV%Q(ICV),CV%MIX_TIME(ICV),CV%CHI_R(ICV),&
      ...
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO JCC=1,CC%NCELL
            ...
            IF (ICV>0) THEN
               ...
               CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(ICV)*CV%Q(ICV)*CV%VOLUME(ICV)
         ...
         ELSE
            ...
            DO JCC=1,CC%NCELL
               ...
               IF (CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY) &
                  CALL GET_FV_CV_FOR_CUTCELL(NM,ICC,JCC,ICV)
               IF (ICV>0) THEN
                  CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(ICV)*CV%VOLUME(ICV)
               ELSEIF (.NOT.(CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY)) THEN
   ...
   IF (CC_IBM) THEN
      ...
      DO NC=1,NCHEM_ACTIVE_CC
         ...
         IF (ICV>0) THEN
            ...
            CV%MIX_TIME(ICV)=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+2,INDX)
            CV%CHI_R(ICV)=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+3,INDX)
         ELSE
