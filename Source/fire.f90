...
USE MESH_POINTERS
USE CC_SCALARS, ONLY: GET_FV_CV_FOR_CUTCELL, GET_CUTCELL_RHOP, GET_CUTCELL_TMP, GET_CUTCELL_ZZV
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IF (CC_IBM) MESHES(NM)%FV%CV%Q=0._EB
ENDDO
...
USE TYPES, ONLY: CC_CV_TYPE

INTEGER :: NM,I,J,K,NC,CHEM_SUBIT_TMP,CHEM_SUBIT_TMP_OUT,ICC,JCC,ICV,INDX,NRECEIVE_CELLS
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IF (REAC_SOURCE_CHECK) THEN
      ...
      Q_REAC=0._EB
      IF (ALLOCATED(MESHES(NM)%FV%CV%Q_REAC)) MESHES(NM)%FV%CV%Q_REAC=0._EB
   ENDIF
   ...
   IF (CC_IBM) THEN
      ...
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO JCC=1,CC%NCELL
            ...
            ENDIF
            IF (ICV>0) THEN
               IF (ABS(CV%VOLUME(ICV)/VCELL)<1.E-12_EB) CYCLE
               ZZ_GET=CV%ZZ(1:N_TRACKED_SPECIES,ICV)
               MYTEMP=CV%TMP(ICV)
            ELSE
               IF (ABS(CUT_CELL(ICC)%VOLUME(JCC)/VCELL)<1.E-12_EB) CYCLE
               CALL GET_CUTCELL_ZZV(NM,ICC,JCC,1._EB,ZZ_GET)
               MYTEMP=GET_CUTCELL_TMP(NM,ICC,JCC)
            ENDIF
            PRES = PBAR(K,PRESSURE_ZONE(I,J,K)) + RHO(I,J,K)*(H(I,J,K)-KRES(I,J,K))
            CALL CHECK_CHEMICALLY_ACTIVE_STATE(ZZ_GET,MYTEMP,I,J,K,.TRUE.,IS_CHEM_ACTIVE,IGN_ZN)
            IF (STOP_STATUS/=NO_STOP) RETURN
            IF (.NOT. IS_CHEM_ACTIVE) CYCLE
            NCHEM_ACTIVE_CELLS_AND_CC = NCHEM_ACTIVE_CELLS_AND_CC + 1
         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               IF (CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY) &
                  CALL GET_FV_CV_FOR_CUTCELL(NM,ICC,JCC,ICV)
               IF (ICV>0) THEN
                  CV%CHI_R(CC%IG(JCC))=0._EB
                  ZZ_GET=CV%ZZ(1:N_TRACKED_SPECIES,ICV)
                  DZZ=ZZ_GET
                  CALL COMBUSTION_MODEL(T,DT,ZZ_GET,CV%Q(ICV),CV%MIX_TIME(CC%IG(JCC)),CV%CHI_R(CC%IG(JCC)),&
                                        CHEM_SUBIT_TMP,REAC_SOURCE_TERM_TMP,Q_REAC_TMP,CV%TMP(ICV),CV%RHO(ICV),PRES,MU(I,J,K),&
                                        LES_FILTER_WIDTH(I,J,K),CV%VOLUME(ICV),ZETA_0,IGN_ZN,IIC=I,JJC=J,KKC=K)
               ELSE
                  CV%CHI_R(CC%IG(JCC))=0._EB
                  CALL GET_CUTCELL_ZZV(NM,ICC,JCC,1._EB,ZZ_GET)
                  DZZ=ZZ_GET
                  MYTEMP=GET_CUTCELL_TMP(NM,ICC,JCC)
                  MYRHO=GET_CUTCELL_RHOP(NM,ICC,JCC,1._EB)
                  CALL COMBUSTION_MODEL(T,DT,ZZ_GET,CV%Q(CC%IG(JCC)),CV%MIX_TIME(CC%IG(JCC)),CV%CHI_R(CC%IG(JCC)),&
                                        CHEM_SUBIT_TMP,REAC_SOURCE_TERM_TMP,Q_REAC_TMP,MYTEMP,MYRHO,PRES,MU(I,J,K),&
                                        LES_FILTER_WIDTH(I,J,K),CC%VOLUME(JCC),ZETA_0,IGN_ZN,IIC=I,JJC=J,KKC=K)
               ENDIF
               CALL SET_SPECIES_SOURCE_TERM_CUTCELL(DT,NM,ICC,JCC,ICV,ZZ_GET,DZZ,Q_REAC_TMP)
            END DO
         ENDIF
         IF(ALLOCATED(DZ_F0)) DEALLOCATE(DZ_F0)
         IF (STOP_STATUS/=NO_STOP) RETURN
      ...
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO JCC=1,CC%NCELL
            ...
            IF (CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY) &
               CALL GET_FV_CV_FOR_CUTCELL(NM,ICC,JCC,ICV)
            IF (ICV>0) THEN
               Q(I,J,K)=Q(I,J,K)+CV%Q(ICV)*CV%VOLUME(ICV)
               CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(CC%IG(JCC))*CV%Q(ICV)*CV%VOLUME(ICV)
            ELSEIF (.NOT.(CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY)) THEN
               Q(I,J,K)=Q(I,J,K)+CV%Q(CC%IG(JCC))*CC%VOLUME(JCC)
               CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(CC%IG(JCC))*CV%Q(CC%IG(JCC))*CC%VOLUME(JCC)
            ENDIF
         ENDDO
         ...
         ELSE
            ...
            DO JCC=1,CC%NCELL
               ...
               IF (CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY) &
                  CALL GET_FV_CV_FOR_CUTCELL(NM,ICC,JCC,ICV)
               IF (ICV>0) THEN
                  CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(CC%IG(JCC))*CV%VOLUME(ICV)
               ELSEIF (.NOT.(CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY)) THEN
                  CHI_R(I,J,K)=CHI_R(I,J,K)+CV%CHI_R(CC%IG(JCC))*CC%VOLUME(JCC)
               ENDIF
            ENDDO
...
SUBROUTINE SET_SPECIES_SOURCE_TERM_CUTCELL(DT,NM,ICC,JCC,ICV,ZZ_NEW,ZZ_OLD,Q_REAC_TMP)
...
REAL(EB), INTENT(IN) :: DT
REAL(EB), INTENT(INOUT) :: ZZ_NEW(N_TRACKED_SPECIES), ZZ_OLD(N_TRACKED_SPECIES)
REAL(EB), INTENT(IN) :: Q_REAC_TMP(N_REACTIONS)
INTEGER, INTENT(IN) :: NM,ICC,JCC,ICV
REAL(EB) :: DZZ(1:N_TRACKED_SPECIES)
REAL(EB) :: CP, H_S_N, TMP_LOC, RHO_LOC, RSUM_LOC
LOGICAL :: Q_EXISTS, REALIZABLE
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM
TYPE(CC_CV_TYPE), POINTER :: CV
INTEGER :: N,IG

IF (REAC_SOURCE_CHECK .AND. ICV>0) MESHES(NM)%FV%CV%Q_REAC(:,ICV)=Q_REAC_TMP
...
DZZ_IF2: IF ( ANY(ABS(DZZ) > DZZ_CLIP) ) THEN
   IF (ICV>0) THEN
      CV => MESHES(NM)%FV%CV
      IF (ABS(CV%Q(ICV)) > TWENTY_EPSILON_EB) Q_EXISTS=.TRUE.
      CALL GET_SPECIFIC_HEAT(ZZ_NEW,CP,CV%TMP(ICV))
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_NEW,CV%RSUM(ICV))
      DO N=1,N_TRACKED_SPECIES
         SM=>SPECIES_MIXTURE(N)
         CALL GET_SENSIBLE_ENTHALPY_Z(N,CV%TMP(ICV),H_S_N)
         CV%D_SOURCE(ICV)=CV%D_SOURCE(ICV)+(SM%RCON/CV%RSUM(ICV)-H_S_N/(CP*CV%TMP(ICV)))*DZZ(N)/DT
         CV%M_DOT_PPP(N,ICV)=CV%M_DOT_PPP(N,ICV)+CV%RHO(ICV)*DZZ(N)/DT
      ENDDO
   ELSE
      CV => MESHES(NM)%FV%CV
      IG = CUT_CELL(ICC)%IG(JCC)
      TMP_LOC = GET_CUTCELL_TMP(NM,ICC,JCC)
      RHO_LOC = GET_CUTCELL_RHOP(NM,ICC,JCC,1._EB)
      CALL GET_SPECIFIC_HEAT(ZZ_NEW,CP,TMP_LOC)
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_NEW,RSUM_LOC)
      IF (IG>=1) THEN
         IF (ABS(CV%Q(IG)) > TWENTY_EPSILON_EB) Q_EXISTS=.TRUE.
         CV%RSUM(IG) = RSUM_LOC
         DO N=1,N_TRACKED_SPECIES
            SM=>SPECIES_MIXTURE(N)
            CALL GET_SENSIBLE_ENTHALPY_Z(N,TMP_LOC,H_S_N)
            CV%D_SOURCE(IG)=CV%D_SOURCE(IG)+ &
               (SM%RCON/RSUM_LOC-H_S_N/(CP*TMP_LOC))*DZZ(N)/DT
            CV%M_DOT_PPP(N,IG)=CV%M_DOT_PPP(N,IG)+RHO_LOC*DZZ(N)/DT
         ENDDO
      ENDIF
   ENDIF
ENDIF DZZ_IF2
...
INTEGER :: I,J,K,NM,INDX,ICC,JCC,ICV,RANK_TO,SEQ,IERR
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (CC_IBM) THEN
         ...
         DO NC=1,NCHEM_ACTIVE_CC
            ...
            IF (ICV>0) THEN
               ZZ_GET=CV%ZZ(1:N_TRACKED_SPECIES,ICV)
               REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+1,INDX)=CV%TMP(ICV)
               REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+2,INDX)=CV%RHO(ICV)
               REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+6,INDX)=CV%VOLUME(ICV)
            ELSE
               CALL GET_CUTCELL_ZZV(NM,ICC,JCC,1._EB,ZZ_GET)
               REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+1,INDX)=GET_CUTCELL_TMP(NM,ICC,JCC)
               REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+2,INDX)=GET_CUTCELL_RHOP(NM,ICC,JCC,1._EB)
...
REAL(EB), INTENT(IN) :: DT
INTEGER :: NCHEM_ACTIVE_CELLS_AND_CC, NCHEM_ACTIVE_CELLS, NCHEM_ACTIVE_CC, NC
INTEGER :: I,J,K,NM,INDX,ICC,JCC,ICV,IERR,SEQ
INTEGER :: TEMPARR1(N_MPI_PROCESSES), TEMPARR2(N_MPI_PROCESSES)
   ...
   IF (CC_IBM) THEN
      ...
      DO NC=1,NCHEM_ACTIVE_CC
         ...
         IF (CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY) &
            CALL GET_FV_CV_FOR_CUTCELL(NM,ICC,JCC,ICV)
         IF (ICV>0) THEN
            DZZ=CV%ZZ(1:N_TRACKED_SPECIES,ICV)
            CV%Q(ICV)=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+1,INDX)
            CV%MIX_TIME(CC%IG(JCC))=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+2,INDX)
            CV%CHI_R(CC%IG(JCC))=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+3,INDX)
         ELSE
            CALL GET_CUTCELL_ZZV(NM,ICC,JCC,1._EB,DZZ)
            CV%Q(CC%IG(JCC))=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+1,INDX)
            CV%MIX_TIME(CC%IG(JCC))=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+2,INDX)
            CV%CHI_R(CC%IG(JCC))=RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+3,INDX)
         ENDIF
         ZZ_GET=RESULTS_TO_RECV_ARRAY(1:N_TRACKED_SPECIES,INDX)
