...
FV=>MESHES(NM)%FV
CV=>FV%CV
USE_FV_STATE=CC_CV_USE_IN_SOLVER .AND. CC_CV_SOLVER_SCOPE==CC_CV_SCOPE_IDENTITY
   ...
   CCVAR_IF: IF (CCVAR(II,JJ,KK,CC_IDCC) > 0) THEN ! we have a cutcell
      ...
      CC_LOOP: DO JCC=JCC_LO,JCC_HI
         ...
         IND_SELECT_2: SELECT CASE(IND)
            ...
            CASE(1)   ! DENSITY
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + RHO_CC*Y_SPECIES * VOL
            CASE(5)   ! TEMPERATURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + (TMP_CC-TMPM)    * VOL
            CASE(9)   ! PRESSURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + &
                  ( PBAR(KK,PRESSURE_ZONE(II,JJ,KK)) + RHO_CC*(H_CC-KRES(II,JJ,KK))  - P_0(KK) ) * VOL
            CASE(10)  ! VELOCITY
               IF(II<1 .OR. II>IBAR .OR. JJ<1 .OR. JJ>JBAR .OR. KK<1 .OR. KK>KBAR) THEN
                  GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + SQRT(2._EB*KRES(II,JJ,KK)) * VOL
               ELSE
                  CALL CC_CUTCELL_VELOCITY(NM,0._EB,ICC,JCC,VELOCITY_COMPONENT,ATOTV,RETURN_INTEGRALS=.FALSE.)
                  GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + SQRT(DOT_PRODUCT(VELOCITY_COMPONENT,VELOCITY_COMPONENT)) * VOL
               ENDIF
            CASE(11)  ! HRRPUV
               GAS_PHASE_OUTPUT_CC=GAS_PHASE_OUTPUT_CC+Q_CC*0.001_EB*VOL
            CASE(12)  ! H
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + 0.5_EB*(H_CC+HS_CC) * VOL
            CASE(14)  ! DIVERGENCE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + D_CC             * VOL
            CASE(15)  ! MIXING TIME
               GAS_PHASE_OUTPUT_CC=GAS_PHASE_OUTPUT_CC+MIX_CC*VOL
            CASE(19)  ! RADIATION LOSS
               GAS_PHASE_OUTPUT_CC=GAS_PHASE_OUTPUT_CC+QR_CC*0.001_EB*VOL
            CASE(22)  ! HS
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + HS_CC            * VOL
            CASE(90)  ! MASS FRACTION
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + Y_SPECIES/(1._EB-Y_H2O)          * VOL
            CASE(94)  ! VOLUME FRACTION
               GAS_PHASE_OUTPUT_CC=GAS_PHASE_OUTPUT_CC+RCON*Y_SPECIES/RSUM_CC/ &
                  (1._EB-R_Y_H2O/RSUM_CC)*VOL
            CASE(138) ! HRRPUV REAC
               IF (ICV>0) GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + CV%Q_REAC(REAC_INDEX,ICV)*0.001_EB*VOL
            CASE(191) ! SCALAR UNKNOWN NUMBER
               GAS_PHASE_OUTPUT_RES = REAL(CUT_CELL(ICC)%UNKZ(JCC),EB); RETURN

            CASE(523) ! ABSOLUTE PRESSURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + &
                  ( PBAR(KK,PRESSURE_ZONE(II,JJ,KK)) + RHO_CC*(H_CC-KRES(II,JJ,KK)) ) * VOL
            CASE(550) ! CUTCELL VELOCITY DIVERGENCE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + CCVELDIV(II,JJ,KK)*VOL
   ...
   AXIS_IF: IF (AXIS>0) THEN
      FCVAR_IF: IF (FCVAR(II,JJ,KK,CC_IDCF,AXIS)>0) THEN
         ...
         CFA_LOOP: DO JCF=1,NFACE
            ...
            IF_=0; IF (USE_FV_STATE) IF_=CUT_FACE(ICF)%FC(JCF)
            IF (IF_>0) THEN
               FN_CF = FV%FACE%FN(IF_)
               VEL_CF = FV%FACE%VEL(IF_)
            ELSE
               FN_CF = CUT_FACE(ICF)%FN(JCF)
               VEL_CF = CUT_FACE(ICF)%VEL(JCF)
            ENDIF
            ...
            IND_SELECT_3: SELECT CASE(IND)
               ...
               CASE( 2, 3, 4)  ! F_X, F_Y, F_Z
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + FN_CF * CUT_FACE(ICF)%AREA(JCF)
               CASE(6)   ! U-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + VEL_CF * CUT_FACE(ICF)%AREA(JCF)
               CASE(7)   ! V-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + VEL_CF * CUT_FACE(ICF)%AREA(JCF)
               CASE(8)   ! W-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + VEL_CF * CUT_FACE(ICF)%AREA(JCF)
               CASE(192:194) ! F_X,F_Y,F_Z UNKNOWN NUMBER
                  IF (IF_>0) THEN
                     GAS_PHASE_OUTPUT_RES = REAL(FV%FACE%UNKF(IF_),EB)
                  ELSE
                     GAS_PHASE_OUTPUT_RES = REAL(CUT_FACE(ICF)%UNKF(JCF),EB)
                  ENDIF
                  RETURN
...
INTEGER :: I_DEPTH,II2,IIG,JJG,KKG,NN,IWX,SURF_INDEX,I,J,NWP,M_INDEX,ICC,IND1,IND2,IC2,ITMP,ICF,JCF,NFACE,NR,&
           MATL_INDEX,OUTPUT_INDEX,IF_
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 3) ! NORMAL VELOCITY
      ...
      IF(PRESENT(OPT_CFACE_INDEX)) THEN
         ...
         IF_=0; IF (USE_FV_STATE) IF_=CUT_FACE(IND1)%FC(IND2)
         IF (IF_>0) THEN
            SOLID_PHASE_OUTPUT = FV%FACE%VEL(IF_)
         ELSE
         ...
         DO JCF=1,NFACE
            ...
            IF (IF_>0) THEN
               SOLID_PHASE_OUTPUT_CTF = SOLID_PHASE_OUTPUT_CTF &
                                      - SIGN(1._EB,REAL(BC%IOR,EB))*FV%FACE%VEL(IF_)*CUT_FACE(ICF)%AREA(JCF)
            ELSE
               SOLID_PHASE_OUTPUT_CTF = SOLID_PHASE_OUTPUT_CTF &
   ...
   CASE(66) ! VISCOUS STRESS WALL (takes optional FORCE_DIRECTION vector)
      ...
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         ...
         IND1 = CFA%CUT_FACE_IND1
         IND2 = CFA%CUT_FACE_IND2
         CALL GET_UVWGAS_CFACE(U_CELL,V_CELL,W_CELL,IND1,IND2,U,V,W,PREDFCT=1._EB,NM=NM)
         ...
         IF (CC_IBM) THEN
            ...
            IF (CCVAR(I,J,K,CC_IDCC) > 0) THEN ! we have a cutcell
               ...
               DO JCC=1,NCELL
                  ...
                  MASS_INTEGRAL(1:N_SPECIES) = MASS_INTEGRAL(1:N_SPECIES) + RHO_CC*Y_MF_INT(1:N_SPECIES)*VC
                  MASS_INTEGRAL(N_SPECIES+1:N_SPECIES+N_TRACKED_SPECIES) = MASS_INTEGRAL(N_SPECIES+1:N_SPECIES+N_TRACKED_SPECIES) &
                     + RHO_CC*ZZ_GET(1:N_TRACKED_SPECIES)*VC
               ENDDO
               CYCLE
            ENDIF
         ENDIF
...
USE COMP_FUNCTIONS, ONLY: GET_FILE_NUMBER
USE CC_SCALARS, ONLY: GET_CUTCELL_ZZP,GET_CUTCELL_RHOP,GET_CUTCELL_H
...
IF (CC_IBM) THEN
   ...
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      DO JCC=1,CUT_CELL(ICC)%NCELL
         ...
         CUT_CELL(ICC)%VOLUME(JCC),',',GET_CUTCELL_ZZP(NM,ICC,JCC,2,1._EB),',',GET_CUTCELL_H(NM,ICC,JCC,1._EB),',',&
         GET_CUTCELL_RHOP(NM,ICC,JCC,1._EB)*(GET_CUTCELL_H(NM,ICC,JCC,1._EB)-KRES(I,J,K))
      ENDDO
   ENDDO
