...
!$OMP PARALLEL PRIVATE(SF,B1,BC,B1,SURF_INDEX,RSUM_G,MU_G,TSI,&
!&                     RAMP_FACTOR,UBAR,VBAR,WBAR,CFA,ICF)

!$OMP DO SCHEDULE(STATIC) PRIVATE(IW,WC)
...
IF (CALL_HT_1D) THEN
   !$OMP DO SCHEDULE(STATIC) PRIVATE(ITW)
...
!$OMP DO SCHEDULE(STATIC) PRIVATE(ICF,CFA)
...
SOLID_PARTICLES_IF:IF (SOLID_PARTICLES) THEN
   !$OMP DO SCHEDULE(STATIC) PRIVATE(IP,LP,LPC,ONE_D,PY,SF_HTC)
   DO IP = 1, NLP
...
!$OMP PARALLEL PRIVATE(MU_G,B1,BC)
...
REAL(EB) :: RADIUS,AREA_SCALING,M_DOT_PPP_SINGLE,M_DOT_SINGLE,CP,MW_RATIO,H_G,&
            ZZ_GET(1:N_TRACKED_SPECIES),DENOM,M_GAS,LENGTH,WIDTH
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
INTEGER :: II,JJ,KK,IIG,JJG,KKG,IW,IC,ICG,ICF,NS,IP,OBST_INDEX
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR_P
...
!$OMP PARALLEL PRIVATE(SF,B1,B2,BC,IC,ICG,II,JJ,KK,IIG,JJG,KKG,OBST_INDEX)

! Add evaporating gases from solid particles to the mesh using a volumetric source term
!$OMP DO SCHEDULE(STATIC) PRIVATE(IP,LP,LPC,RADIUS,AREA_SCALING,LENGTH,WIDTH,ONE_D,&
!&                                NS,M_GAS,MW_RATIO,M_DOT_SINGLE,ZZ_GET,DENOM,H_S_B,CP,H_G)
...
REAL(EB) :: T_SCALE(MAX_QDOTPP_REF),QDOTPP_REF(MAX_QDOTPP_REF),QDOTPP_T(MAX_QDOTPP_REF), &
            QDOTPP,QDOTPP1,QDOTPP2,DT_SPYRO(MAX_QDOTPP_REF),RVC
INTEGER :: N,IDX1,IDX2,NQ,ITER,IIO,JJO,KKO,OTHER_MESH_OBST_INDEX,LL,SPECIES_BC_INDEX,IOR
