...
REAL(EB) :: AGL_SLICE
INTEGER :: N,N1,NN,NM,MESH_NUMBER,NITER,ITER,VELO_INDEX,GEOM_INDEX,IOR
...
CHARACTER(MESSAGE_LENGTH) :: SLCF_NAME
CHARACTER(MESSAGE_LENGTH), ALLOCATABLE, DIMENSION(:) :: UNIQUE_SLICE_NAMES_TMP
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   IF (.NOT.ALLOCATED(UNIQUE_SLICE_NAMES_TMP)) ALLOCATE(UNIQUE_SLICE_NAMES_TMP(N_SLCF_O*4))
   CALL COUNT_UNIQUE_SLCF(N_SLCF_O,N_UNIQUE_SLCF,UNIQUE_SLICE_NAMES_TMP)

   ALLOCATE(UNIQUE_SLICE_NAMES(N_UNIQUE_SLCF)); UNIQUE_SLICE_NAMES(1:N_UNIQUE_SLCF) = UNIQUE_SLICE_NAMES_TMP(1:N_UNIQUE_SLCF)
   DEALLOCATE(UNIQUE_SLICE_NAMES_TMP)
   ALLOCATE(M%EMPTY_UNIQUE_SLICE(N_UNIQUE_SLCF)); M%EMPTY_UNIQUE_SLICE(:) = .TRUE.
   ALLOCATE(UNIQUE_SLICE_IS_SL3D(N_UNIQUE_SLCF)); UNIQUE_SLICE_IS_SL3D(:) = .FALSE.

   IF (.NOT.ALLOCATED(ALL_SLICE_NAMES)) THEN
      ALLOCATE(ALL_SLICE_NAMES(N_SLCF_O*4),ALL_SLICE_QUANTITIES(N_SLCF_O*4),ALL_SLICE_TOPOLOGIES(N_SLCF_O*4))
      ALL_SLICE_NAMES(:)=""
      ALL_SLICE_QUANTITIES(:)=""
      ALL_SLICE_TOPOLOGIES(:)=-1
   ENDIF
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      DO ITER=1,NITER
         N1 = N1 + 1
         ALL_SLICE_NAMES(N1) = TRIM(SLCF_NAME)
         ALL_SLICE_TOPOLOGIES(N1)=1; IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) ALL_SLICE_TOPOLOGIES(N1) = 0
         ...
         CALL GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,INDEX,INDEX2, &
                                 ...
                                 SLICETYPE=SLICETYPE)
         ALL_SLICE_QUANTITIES(N1) = SMOKEVIEW_LABEL
      ENDDO
      DO I = 1,N_UNIQUE_SLCF
         IF (TRIM(SLCF_NAME)==TRIM(UNIQUE_SLICE_NAMES(I))) THEN
            IF (ALL_SLICE_TOPOLOGIES(N1)==1) UNIQUE_SLICE_IS_SL3D(I)=.TRUE.
         ENDIF
      ENDDO
