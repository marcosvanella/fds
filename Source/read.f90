...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE, &
                BNDF_DEFAULT,CC_IBM,CC_USE_SET_CVS_3D,&
                CC_CV_USE_IN_SOLVER,CC_CV_SOLVER_SCOPE,CCVOL_LINK,C_DEARDORFF,&
...
! CV solver scope is the single user knob for agglomeration reach. Cross-mesh linking
! follows SCOPE==2 (setup-only topology studies use SCOPE=2 with USE_IN_SOLVER=.FALSE.).
IF (CC_CV_SOLVER_SCOPE < CC_CV_SCOPE_IDENTITY .OR. CC_CV_SOLVER_SCOPE > CC_CV_SCOPE_CROSS_MESH) THEN
   WRITE(MESSAGE,'(A,I0)') 'ERROR: CC_CV_SOLVER_SCOPE must be 0, 1, or 2; got ',CC_CV_SOLVER_SCOPE
   CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
IF (CC_CV_USE_IN_SOLVER .AND. .NOT.CC_USE_SET_CVS_3D) THEN
   WRITE(MESSAGE,'(A)') &
      'ERROR: CC_CV_USE_IN_SOLVER requires CC_USE_SET_CVS_3D so control volumes are defined at setup.'
   CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
CC_CV_ENABLE_CROSS_MESH_LINKING = (CC_CV_SOLVER_SCOPE == CC_CV_SCOPE_CROSS_MESH)
